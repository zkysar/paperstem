...
export async function compressToMp3(
  ...
  const encoder = new Encoder(channels, sampleRate, TARGET_BITRATE_KBPS);
  ...
  for (let i = 0; i < length; i += FRAME_SAMPLES) {
    ...
    const leftFrame = left.subarray(i, end);
    const rightFrame = right ? right.subarray(i, end) : undefined;

    const mp3buf = rightFrame
      ? encoder.encodeBuffer(leftFrame, rightFrame)
